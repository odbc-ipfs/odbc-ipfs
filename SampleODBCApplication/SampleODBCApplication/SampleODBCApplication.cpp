...
SQLHDBC conn(SQLHENV *env) {
    ...
    wchar_t str_dsn[] = L"FILEDSN=C:/Users/Thomas/Documents/TestodbcDSN/test2.dsn;";
    //char str_dsn[] = "FILEDSN=C:/Users/Thomas/Documents/TestodbcDSN/odbcdsntest.dsn;";

    SQLAllocHandle(SQL_HANDLE_DBC, *env, &dbc);
    ret = SQL_SUCCESS;
    ret = SQLDriverConnect(dbc, NULL, str_dsn, SQL_NTS, str_ret, sizeof(4096), &str_ret_len, SQL_DRIVER_COMPLETE);
    //ret = SQLDriverConnectA(dbc, NULL, (SQLCHAR*) str_dsn, SQL_NTS, str_ret, sizeof(4096), &str_ret_len, SQL_DRIVER_NOPROMPT);

    
    //ret = SQLConnect(dbc, str_dsn, SQL_NTS, NULL, 0, NULL, 0);
...
void getData2(SQLHDBC dbc) {
    ...
    SQLWCHAR* colTitle[64];
    SQLSMALLINT strLen = 1;
    ret = SQLColAttribute(stmt, 1, SQL_DESC_NAME, colTitle, sizeof(colTitle), &strLen, NULL);
    
    SQLINTEGER intData = 0;


    ret = SQLBindCol(stmt, 1, SQL_INTEGER, (SQLPOINTER)intData, sizeof(intData), NULL); //&i

    ret = SQLFetch(stmt);

    wprintf(L"Row 1: Col: 1     Title: %ls, Value: %d", (SQLWCHAR*) colTitle, intData);
    */
}
